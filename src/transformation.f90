module transformation
  ...
  private :: T_a12_IndvAngles, T_a12_ArrayAngles, &
             T_r21_IndvAngles, T_r21_ArrayAngles
  ...
  interface T_r21
    module procedure T_r21_ArrayAngles
    module procedure T_r21_IndvAngles
  end interface T_r21
  ...
  contains
    ...
    pure function T_r21_ArrayAngles(angles) result(T_r)
      real*8 :: T_r(3,3)
      real*8, intent(in) :: angles(3)
      real*8 :: cosphi, costheta, cospsi, &
                sinphi, sintheta, sinpsi, tantheta
      costheta = cos(angles(2))
      cospsi = cos(angles(3))
      tantheta = tan(angles(2))
      sinpsi = sin(angles(3))


      T_r(1,:) = (/ 1d0, sinpsi*tantheta, cospsi*tantheta /)
      T_r(2,:) = (/ 0d0,          cospsi,         -sinpsi /)
      T_r(3,:) = (/ 0d0, sinpsi/costheta, cospsi/costheta /)

    end function T_r21_ArrayAngles

    pure function T_r21_IndvAngles(theta, psi) result(T_r)
      real*8 :: T_r(3,3)
      real*8, intent(in) :: theta, psi
      real*8 :: cosphi, costheta, cospsi, &
                sinphi, sintheta, sinpsi, tantheta
      costheta = cos(theta)
      cospsi = cos(psi)
